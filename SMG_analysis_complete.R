...
for (strain in ref_strains) {
  ...
  for (condition in ref_conditions) {
    ...
    hgt_set_data <- hm_DEG_mat %>%
      ...
      group_by(ref_strain, ref_sub, strain_condition, pathway) %>%
  ...
  # Heatmap output
  hm_file_name <- file.path(out_dir, paste0("heatmap_SMG_ref_", strain, ".", export_file))
...
for (strain in ref_strains) {
  ...
  for (deg_type in deg_types) {
    ...
    p <- plot_data %>%
      ggplot(aes(y = pathway, x = strain_condition, size = Significant, color = logFDR)) +
      ...
      labs(
        title = paste0("Gene Set Enrichment for sugar metabolism ", deg_type, " genes (Ref: ",strain,")"),
        y = "Predicted pathway",
