...
strains <- c("CBS_464","bmtr3_35.2")
conditions <- gsub(paste(paste0(strains,"_"), collapse = "|"), "", strain_condition_order) %>%
  unique()

# Load aggregated GO enrichment result
go_data <- read.csv(input_file) %>%
  mutate(
    sample_cond = gsub(paste(paste0(strains,"_"), collapse = "|"), "", strain_condition),
    ref_cond = gsub(paste(paste0(strains,"_"), collapse = "|"), "", ref_condition),
    sample_strain = gsub(paste(paste0("_",conditions), collapse = "|"), "", strain_condition),
    ref_strain = gsub(paste(paste0("_",conditions), collapse = "|"), "", ref_condition),
    strain_condition = factor(strain_condition, levels = strain_condition_order),
    ref_condition = factor(ref_condition, levels = strain_condition_order),
...
# Loop through the deg type
for (ref_strain in strains) {
  go_ref_data <- go_data %>%
    filter(ref_strain == !!ref_strain,
           sample_strain != !!ref_strain,
           ref_cond == sample_cond)
  ...
  for (deg_type in deg_types) {
    ...
    if (nrow(plot_data) == 0) {
      ...
      p <-  plot_data %>%
        ...
        # Improve axis labels and theme
        labs(
          title = paste0("GO Enrichment for ", deg_type, " Genes (Ref: ",ref_strain,")"),
          y = "GO Term",
          x = "Strain and Condition"
        ) +
        theme_minimal(base_size = 12) +
      ...
      # Save plot
      ggsave(
        filename = file.path(output_dir, paste0("topGO_enrichment_plot_ref_",ref_strain,"_",deg_type,".", export_file)),
        plot = p,
        width = 20,
        height = 10,
        device = export_file)
...
for (ref_cond in conditions) {
  go_ref_data <- go_data %>%
    filter(ref_cond == !!ref_cond,
           sample_cond != !!ref_cond,
  ...
  for (deg_type in deg_types) {
    ...
    if (nrow(plot_data) == 0) {
      ...
      p <-  plot_data %>%
        ...
        # Improve axis labels and theme
        labs(
          title = paste0("GO Enrichment for ", deg_type, " Genes (Ref: ",ref_cond,")"),
          y = "GO Term",
          x = "Strain and Condition"
        ) +
        theme_minimal(base_size = 12) +
      ...
      # Save plot
      ggsave(
        filename = file.path(output_dir, paste0("topGO_enrichment_plot_ref_",ref_cond,"_",deg_type,".", export_file)),
        plot = p,
        width = 20,
        height = 10,
        device = export_file)
