...
for (condition in ref_conditions) {
  ...
  # Heatmap output
  hm_file_name <- file.path(out_dir, paste0("heatmap_ST_ref_", condition, ".pdf"))
  ...
  hgt_set_data <- hm_DEG_mat %>%
    ...
    group_by(ref_sub, strain_condition, specificity) %>%
...
enrichment_data <- enrichment_results %>%
  ...
  mutate(specificity = factor(specificity, levels = specificity_order))

for (substrate in ref_conditions) {
  p_data <- enrichment_data %>%
    filter(ref_sub == substrate) 
  p <-  p_data %>%
    ggplot(aes(x = specificity, y = strain_condition, size = -log10(padj_oe), color = set_oeg)) +
