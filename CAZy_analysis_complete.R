...
# condition <- ref_conditions[2] # for debugging purposes
# strain <- ref_strains[1] # for debugging purposes
...
for (substrate_set_name in names(substrate_list)) {
  ...
  for (strain in ref_strains) {
    message("Generating heatmap for ", substrate_set_name, " with ref ", strain)
    ...
    for (condition in ref_conditions) {
      ...
      hgt_total_data <- DEG_mat %>%
        ...
        group_by(gene_set, ref_strain, ref_sub, strain_condition) %>%
    ...
    # Heatmap output
    hm_file_name <- file.path(out_dir, paste0("heatmap_", substrate_set_name, "_ref_", strain, ".", export_file))
...
for (strain in ref_strains) {
  ...
  for (deg_type in deg_types) {
    ...
    p <-  plot_data %>%
      ...
      labs(
        title = paste0("Gene Set Enrichment for CAZy", deg_type, " genes (Ref: ",strain,")"),
        y = "CAZy set",
