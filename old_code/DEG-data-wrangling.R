...
required_dirs <- c(tidy_data_dir, qc_dir, dds_dir, deg_dir)
...
all_norm <- read.csv(file.path(dds_dir, "norm_counts_CBS_464_all_strains.csv")) %>%
...
write.csv(all_norm, file = file.path(out_dir,"tidy_all_strains_norm.csv"))
...
all_norm_avg <- all_norm %>%
  pivot_longer(cols = -protein_id, names_to = "sample", values_to = "norm_counts") %>%
  left_join(all_labels) %>%
  group_by(protein_id,strain_condition) %>%
  dplyr::summarise(mean_norm_counts = mean(norm_counts)) %>%
  ungroup() %>%
  mutate(strain_condition = gsub("-",".",strain_condition)) %>%
  pivot_wider(names_from = strain_condition, values_from = mean_norm_counts) %>%
  column_to_rownames("protein_id")
...
write.csv(all_norm_avg, file = file.path(out_dir,"tidy_all_strains_norm_avg.csv"))
