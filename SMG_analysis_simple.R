...
for (condition in ref_conditions) {
  ...
  # Heatmap output
  hm_file_name <- file.path(out_dir, paste0("heatmap_SMG_ref_", condition, ".", export_file))
...
for (substrate in ref_conditions) {
  ...
  for (deg_type in deg_types) {
    ...
    p <- plot_data %>%
      ...
      labs(
        title = paste0("Gene Set Enrichment for sugar metabolism ", deg_type, " genes (Ref: ",substrate,")"),
        y = "Predicted pathway",
